...
namespace :live do
  ...
  task :start, [:port] do |t, args|
    system "/usr/bin/ruby -E utf-8:utf-8 /usr/bin/thin -C config/thin-production.yaml -o #{args.port} start" if args.port
    system '/usr/bin/ruby -E utf-8:utf-8 /usr/bin/thin -C config/thin-production.yaml start' unless args.port
  end
